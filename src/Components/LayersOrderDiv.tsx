...
export function LayersOrderDiv(props: { closeLayerOrderPanel: Setter<boolean> }): JSX.Element {
    ...
    function drop(e: DragEvent) {
        ...
        for (let i = 0; i < layers.length; i++) {
            if (e.dataTransfer?.getData("text") === layers.get(i).uid) {
                draggedLayer = layers.get(i);
            }
            if ((e.currentTarget as HTMLLIElement).id === layers.get(i).uid) {
                dropLayer = layers.get(i);
            }
        }
        if (draggedLayer.uid === dropLayer.uid) {
            return;
        }
        ...
        if (selectedLayer() != undefined && layerOrderListRef) {
            for (const layerEntry of layerOrderListRef.children) {
                if (layerEntry.id == selectedLayer().uid.toString()) {
                    ...
                    layerEntry.children[0].children[1].checked = true;
                } else {
                    // Get rid of UI changes for anything that is no longer selected.
                    layerEntry.children[0].children[1].checked = false;
