...
import { BLUE_TILE_STYLE, HOME_POSITION, TRANSPARENT_TILE_STYLE, WES_3D_EVENTS } from "./Constants";
...
const load = async function (mapState: MapState): Promise<Viewer> {
    ...
    Camera.DEFAULT_VIEW_RECTANGLE = Rectangle.fromDegrees(-90, -90, 90, 90);
    ...
    function addChangeListeners() {
        ...
        window.addEventListener("mapStateChanged", () => {
            ...
            currentDatasourceOptions.forEach((uid: string) => {
                let remove = true;
                for (let i = 0; i < mapState.dataSources.length; i++) {
                    if (mapState.dataSources[i].uid === uid) {
                        remove = false;
                        break;
                    }
                }
                if (remove) {
                    for (let i = 0; i < (viewer.dataSources as WesDatasources)._dataSources.length; i++) {
                        if ((viewer.dataSources as WesDatasources)._dataSources[i].uid === uid) {
                            viewer.dataSources.remove((viewer.dataSources as WesDatasources)._dataSources[i]);
                            break;
                        }
                    }
                }
            ...
            currentImageryOptions.forEach((uid: string) => {
                let remove = true;
                for (let i = 0; i < mapState.imageLayers.length; i++) {
                    if (mapState.imageLayers[i].uid === uid) {
                        remove = false;
                        break;
                    }
                }
            ...
            currentTilesetOptions.forEach((uid: string) => {
                let remove = true;
                for (let i = 0; i < mapState.primitiveLayers.length; i++) {
                    if (mapState.primitiveLayers[i].uid === uid) {
                        remove = false;
                        break;
                    }
                }
    ...
    async function addDataSource(dataSourceOption: WesDataSourceObject) {
        ...
        switch (type) {
            ...
            case "celestial":
                ...
                break;
            case "geojson":
                createdDataSource = new GeoJsonDataSource(dataSourceOption.name);
            ...
            case "coverage":
                ...
                createdDataSource._name = `${dataSourceOption.id}: ${dataSourceOption.name}`;
                break;
            case null:
