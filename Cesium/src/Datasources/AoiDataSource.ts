...
import {
    ...
    BUFFER_COLOR,
    DO_RECTANGLE_AOI_BUFFERS
} from "../Constants";
import { CesiumWindow, GeoJsonAoi } from "../Wes";
...
export default class AoiDataSource extends WesDataSource {
    ...
    _aoiPrimitives: PrimitiveCollection;

    constructor(viewer: Viewer) {
        super("Area of Interest", "AOI", "", viewer, AOI_DATASOURCE_ID);
        this._aoiPrimitives = new PrimitiveCollection({ destroyPrimitives: true });
    ...
    async loadService() {
        ...
        entities.removeAll();
        const groundPrims = (window as CesiumWindow).Map3DViewer.scene.groundPrimitives;
        if (this._aoiPrimitives) {
            groundPrims.remove(this._aoiPrimitives);
            if (!this._aoiPrimitives.isDestroyed()) this._aoiPrimitives.destroy();
        }
        this._aoiPrimitives = new PrimitiveCollection({ destroyPrimitives: true });
            ...
            case "Point": {
                ...
                process.nextTick(() => {
                    ...
                    drawables.primitives.forEach(p => {
                        this._aoiPrimitives.add(p);
                    });
                    groundPrims.add(this._aoiPrimitives);
                });
                break;
            }
            case "MultiPoint": {
